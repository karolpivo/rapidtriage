...
#		Windows, OSX, and Linux systems into the user provided <filename>.
#		The script has been organized so that commands or log files used to collect the
#		information can be easily modifed or removed as necaasry. The script also
#		allows the user to select sections to be chosen at runtime. Finally, if necassary, the
#		the results file can be hashed "<filename>-hash" to help ensure the integrity of the
#		results file.
#

import sys,subprocess,os,time,platform,hashlib
from optparse import OptionParser,OptionGroup

# Idenitify the Operating System Type
...
# Setup argument and syntax help
parser=OptionParser(usage='Usage: python %prog [argument(s)] -o <filename>')
parser.add_option('-o',help='specified file will contain the results',dest='outfile')
group1 = OptionGroup(parser, "Arguments (one or more required)")
...
group1.add_option('-l','--log_events',action='store_true', help='collect history and log data', dest='logs')
group1.add_option('-n','--net_stats',action='store_true', help='collect network related information', dest='network')
group1.add_option('-p','--process',action='store_true', help='collect process and service related information', dest='process')
group1.add_option('-t','--sched_tasks',action='store_true', help='collect scheduled task related information', dest='tasks')
group1.add_option('-u','--user',action='store_true', help='collect user related information', dest='user')
parser.add_option_group(group1)
group2 = OptionGroup(parser, "Optional")
group2.add_option('-m','--md5sum',action='store_true', help='hash the results file - output to <filename>-hash', dest='hash')
...
# Provide a timestamp when necassary
def timestamp():
	now = "["+time.strftime("%H:%M:%S")+"]"
	return now

# Execute given commonds and write the results to the user specified outfile
...
# The format is <Description::Commnand> and both are required. Note the double colon "::" is the seperator.
if os_type is "linux":
	cmds = [
...
# are different from the other sections and thus requied the execution for the above commands to be done here. 
for cmd in cmds:
	split_cmd=cmd.split("::")
...
# Linux, Windows,and OSX General System  Related Commands
#
...
# Agian the format for the results of these commands are different than the rest of the program. This requires the execution of the commands here.
for cmd in cmds:
	split_cmd=cmd.split("::")
	outputfile.write("\t"+split_cmd[0]+":\n")
	outputfile.write("\t=========================\n\n")
...
##############################################
	""")
		...
		# Linux, Windows,and OSX Process, Service, or Kernel Module  Related Commands
		#
		#	This list can easily be modified to include additional commands. Use the "<description>::<command>" format.
		#
		#	|	|	|
	...
	if os_type is "linux":
		...
		'Installed Kernel Modules::kextstat',
		]
...
##############################################
	""")
		...
		# Linux, Windows,and OSX Unusual File, Directories, or Registry Key  Related Commands
		#
		#	This list can easily be modified to include additional commands. Use the "<description>::<command>" format.
		#
		#	|	|	|
...
##############################################
	""")
		...
		# Linux, Windows,and OSX Task Related Commands
		#
		#	This list can easily be modified to include additional commands. Use the "<description>::<command>" format.
		#
		#	|	|	|
...
##############################################
	""")
		...
		# Linux, Windows,and OSX Account Related Commands
		#
		#	This list can easily be modified to include additional commands. Use the "<description>::<command>" format.
		#
		#	|	|	|
	...
	if os_type is "linux":
    		...
    		'/etc/shadow Contents::cat /etc/shadow',
